...
permalink: /os_labs/lab2_vulnerableprog
key: labs-cli
license: false
layout: article
nav_key: os_labs
...
It contains the password of the `usernm` using the `$type$salt$hashed` format. 
1.`$type` is the method cryptographic hash <span style="color:#f77729;"><b>algorithm</b></span> and can have the following values:
* `$1$` – MD5
* `$2a$` – Blowfish
* `$y$` – yescrypt
* `$5$` – SHA-256
...
- Remove existing `userfile.txt` if any (from previous loop)
- Then, create `userfile.txt` (this line can be replaced by any other instructions that create this `userfile.txt`)
- Then, runs 3 commands in <span style="color:#f77729;"><b>succession</b></span>:
